...
SELECT emp.emp_no, CONCAT(emp.first_name,' ',emp.last_name) fullname, dt.dept_name
FROM dept_emp d_e
...
FROM departments dt
	...
	INNER JOIN employees emp
		ON dm.emp_no = emp.emp_no
WHERE dm.to_date != DATE(99990101); -- 현재 역임하는 사람은 빼고 출력
		
-- 9.현재 각 직급별 평균월급 중 60,000이상인 직급의 직급명, 평균월급(정수)를 내림차순으로 출력해 주세요.
SELECT 
	tl.title
	,TRUNCATE(AVG(sal.salary),0) avg_s
FROM titles tl
	INNER JOIN salaries sal
		ON tl.emp_no = sal.emp_no
WHERE sal.to_date >= NOW() 
	AND tl.to_date >= NOW()
GROUP BY tl.title HAVING avg_s >= 60000
ORDER BY AVG(sal.salary) DESC;

-- 10.성별이 여자인 사원들의 직급별 사원수를 출력해 주세요.
SELECT tl.title 직급, COUNT(*) 사원수
FROM employees emp
	INNER JOIN titles tl
